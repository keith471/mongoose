...
function SchemaType(path, options, instance) {
  ...
  /*
   * KCS - an array of objects of the following form
   * {validator: `function`, message: `message`, type: 'required'}
   */
  this.validators = [];
  this.setters = [];
  this.getters = [];
...
// KCS - called from document.$__validate which is called by document.validate
// val is the value to validate
// fn is the callback, which only accepts an error (function(err) {...})
SchemaType.prototype.doValidate = function(value, fn, scope) {
  var err = false, //** will be set to true if there is an error with any of the validators for the schematype
      path = this.path,
      count = this.validators.length;

  if (!count) {
    return fn(null); //** nothing to validate for this path, return with no error
  }

  //** this function...
  // if ok is null, then there was an error
  // ok is normally a boolean
  // validator properties: e.g. {message: '`{VALUE}` is required', type:'required', path: 'name', value: 'the value of the field'}
  var validate = function(ok, validatorProperties) {
    if (err) { // KCS - refers to above error
      return;
    }
    if (ok === undefined || ok) {
      --count || fn(null); //** decrement count. If it becomes 0, then we are done with the validators for this field of the schema and thus call the callback with no error
    } else {
      err = new ValidatorError(validatorProperties); //** create an error
      fn(err); //** return with the error
    }
  };

  var _this = this; //** _this refers to the current schematype
  this.validators.forEach(function(v) { //** v is the current `validator` in validators
    if (err) { 
      return; 
    }

    var validator = v.validator; //** validator gets the function defined in {validator: ..., message: ...}

    var validatorProperties = utils.clone(v); //** clones {validator: ..., message: ...} minus the function (validator), so validator properties might end up being {message:..., type:...} for example
    validatorProperties.path = path;
    validatorProperties.value = value;
    // validatorPropertes: {message, type, path, value}

    if (validator instanceof RegExp) { //** validator is a regular expression
      validate(validator.test(value), validatorProperties); //** call the above validate function with true if value meets the regexp and false otherwise, plus the validator properties
                                                            //** And that's it. Then this function returns.
    } else if (typeof validator === 'function') { //** validator is a function
      if (value === undefined && !_this.isRequired) { //** nothing set for this value, but that's fine because it's not required
        validate(true, validatorProperties);
        return;
      }
      if (validator.length === 2) { //**end What's happening here?? Figure out what validate can be and what a length of two means
...
// KCS - called by document.validateSync
